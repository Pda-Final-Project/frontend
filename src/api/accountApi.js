...
const fetchAvailBalance = () =>
  api.get(`account/available-balance`, {
    isAuthRequired: true,
    skipInterceptor: true,
  });
...
const fetchTotalHoldings = () =>
  api.get(`holdings/total/summary`, {
    isAuthRequired: true,
  });

//손익 내역 조회(list)
const fetchTradeProfitList = () =>
  api.get(`tradeProfit/list`, {
    isAuthRequired: true,
  });

//손익 내역 조회(sum)
const fetchTradeProfitSum = () =>
  api.get(`tradeProfit/sum`, {
    isAuthRequired: true,
  });

//보유 종목들 세부 데이터
const fetchHoldings = (sortBy) =>
  api.get(`holdingStocks?sortBy=${sortBy}`, {
    isAuthRequired: true,
    skipInterceptor: true,
  });
